...
import CommunitiesGroupsApi "mixins/communities-groups-api";
import CommunitiesGroupsTypes "types/communities-groups";

actor {
  let accessControlState : AccessControl.AccessControlState;
  include MixinAuthorization(accessControlState, null);
  include MixinObjectStorage();
  let communitiesGroupsState : CommunitiesGroupsTypes.CommunitiesGroupsState;
  include CommunitiesGroupsApi(communitiesGroupsState);
  ...
  // ---- OQL flatten helpers (project nested maps into flat rows) ----

  func flattenFollowing() : [(Principal, Principal)] {
    let out = List.empty<(Principal, Principal)>();
    for ((follower, m) in following.entries()) {
      for ((followee, _) in m.entries()) {
  ...
  func flattenLikes() : [(Nat, Principal)] {
    let out = List.empty<(Nat, Principal)>();
    for ((postId, m) in postLikes.entries()) {
      for ((liker, _) in m.entries()) {
        out.add((postId, liker));
      };
    };
    out.toArray();
  };

  func flattenHashtags() : [(Text, Nat)] {
    let out = List.empty<(Text, Nat)>();
    for ((tag, m) in hashtagIndex.entries()) {
      for ((postId, _) in m.entries()) {
        out.add((tag, postId));
      };
    };
    out.toArray();
  ...
  func flattenNotifications() : [(Principal, Notification)] {
    let out = List.empty<(Principal, Notification)>();
    for ((owner, m) in userNotifications.entries()) {
      for ((_, n) in m.entries()) {
        out.add((owner, n));
      };
    };
    out.toArray();
  ...
  func flattenBlocks() : [(Principal, Principal)] {
    let out = List.empty<(Principal, Principal)>();
    for ((blocker, m) in blocks.entries()) {
      for ((blocked, _) in m.entries()) {
        out.add((blocker, blocked));
      };
    };
    out.toArray();
  };

  func flattenMutes() : [(Principal, Principal)] {
    let out = List.empty<(Principal, Principal)>();
    for ((muter, m) in mutes.entries()) {
      for ((muted, _) in m.entries()) {
        out.add((muter, muted));
      };
    };
    out.toArray();
  ...
  func flattenReplies() : [(Nat, Nat)] {
    let out = List.empty<(Nat, Nat)>();
    for ((parentId, m) in postReplies.entries()) {
      for ((replyId, _) in m.entries()) {
        out.add((parentId, replyId));
      };
    };
    out.toArray();
  };

  func flattenReposts() : [(Nat, Principal)] {
    let out = List.empty<(Nat, Principal)>();
    for ((postId, m) in postReposts.entries()) {
      for ((reposter, _) in m.entries()) {
        out.add((postId, reposter));
      };
    };
    out.toArray();
  };

  func flattenGroupMembers() : [(Nat, Principal)] {
    let out = List.empty<(Nat, Principal)>();
    for ((groupId, m) in communitiesGroupsState.groupMembers.entries()) {
      for ((principal, _) in m.entries()) {
        out.add((groupId, principal));
      };
    };
    out.toArray();
  };
  ...
  include Expose({
    entities = [
      OQL.Entity.manual<(Principal, UserProfile)>("profile", func () = userProfiles.entries(), "Profile", "principal")
        .payload("principal", func ((p, _)) = p)
        .payload("username", func ((_, r)) = r.username)
        .payload("displayName", func ((_, r)) = r.displayName)
        .payload("bio", func ((_, r)) = r.bio)
        .payload("createdAt", func ((_, r)) = r.createdAt)
        .payload("updatedAt", func ((_, r)) = r.updatedAt)
        .ownedBy("principal")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Nat, Post)>("post", func () = posts.entries(), "Post", "id")
        .payload("id", func ((id, _)) = id)
        .payload("author", func ((_, r)) = r.author)
        .payload("text", func ((_, r)) = r.text)
        .payload("createdAt", func ((_, r)) = r.createdAt)
        .public_()
        .build(),
      OQL.Entity.manual<(Principal, Nat)>("userPostCount", func () = userPostCounts.entries(), "UserPostCount", "principal")
        .payload("principal", func ((p, _)) = p)
        .payload("count", func ((_, n)) = n)
        .controllerOnly()
        .build(),
      OQL.Entity.manual<(Principal, Principal)>("follow", func () = flattenFollowing().values(), "Follow", "follower")
        .payload("follower", func ((f, _)) = f)
        .payload("followee", func ((_, e)) = e)
        .ownedBy("follower")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Nat, Principal)>("like", func () = flattenLikes().values(), "Like", "postId")
        .payload("postId", func ((p, _)) = p)
        .payload("liker", func ((_, l)) = l)
        .ownedBy("liker")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Text, Nat)>("hashtag", func () = flattenHashtags().values(), "Hashtag", "tag")
        .payload("tag", func ((t, _)) = t)
        .payload("postId", func ((_, p)) = p)
        .public_()
        .build(),
      OQL.Entity.manual<(Principal, Principal)>("block", func () = flattenBlocks().values(), "Block", "blocker")
        .payload("blocker", func ((b, _)) = b)
        .payload("blocked", func ((_, d)) = d)
        .ownedBy("blocker")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Principal, Principal)>("mute", func () = flattenMutes().values(), "Mute", "muter")
        .payload("muter", func ((m, _)) = m)
        .payload("muted", func ((_, d)) = d)
        .ownedBy("muter")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Nat, Nat)>("reply", func () = flattenReplies().values(), "Reply", "parentPostId")
        .payload("parentPostId", func ((p, _)) = p)
        .payload("replyPostId", func ((_, r)) = r)
        .public_()
        .build(),
      OQL.Entity.manual<(Nat, Principal)>("repost", func () = flattenReposts().values(), "Repost", "postId")
        .payload("postId", func ((p, _)) = p)
        .payload("reposter", func ((_, r)) = r)
        .ownedBy("reposter")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Nat, CommunitiesGroupsTypes.Community)>("community", func () = communitiesGroupsState.communities.entries(), "Community", "id")
        .payload("id", func ((id, _)) = id)
        .payload("name", func ((_, r)) = r.name)
        .payload("description", func ((_, r)) = r.description)
        .payload("creator", func ((_, r)) = r.creator)
        .payload("createdAt", func ((_, r)) = r.createdAt)
        .public_()
        .build(),
      OQL.Entity.manual<(Nat, CommunitiesGroupsTypes.Group)>("group", func () = communitiesGroupsState.groups.entries(), "Group", "id")
        .payload("id", func ((id, _)) = id)
        .payload("communityId", func ((_, r)) = r.communityId)
        .payload("name", func ((_, r)) = r.name)
        .payload("description", func ((_, r)) = r.description)
        .payload("creator", func ((_, r)) = r.creator)
        .payload("createdAt", func ((_, r)) = r.createdAt)
        .public_()
        .build(),
      OQL.Entity.manual<(Nat, Principal)>("groupMember", func () = flattenGroupMembers().values(), "GroupMember", "groupId")
        .payload("groupId", func ((g, _)) = g)
        .payload("member", func ((_, m)) = m)
        .ownedBy("member")
        .controllerOrScoped()
        .build(),
      OQL.Entity.manual<(Nat, CommunitiesGroupsTypes.GroupPost)>("groupPost", func () = communitiesGroupsState.groupPostsById.entries(), "GroupPost", "id")
        .payload("id", func ((id, _)) = id)
        .payload("groupId", func ((_, r)) = r.groupId)
        .payload("author", func ((_, r)) = r.author)
        .payload("text", func ((_, r)) = r.text)
        .payload("createdAt", func ((_, r)) = r.createdAt)
        .public_()
        .build(),
      OQL.Entity.manual<(Principal, Notification)>("notification", func () = flattenNotifications().values(), "Notification", "id")
        .payload("id", func ((_, n)) = n.id)
        .payload("owner", func ((o, _)) = o)
        .payload("actorPrincipal", func ((_, n)) = n.actorPrincipal)
        .payload("actorUsername", func ((_, n)) = n.actorUsername)
        .payload("createdAt", func ((_, n)) = n.createdAt)
        .payload("isRead", func ((_, n)) = n.isRead)
        .ownedBy("owner")
        .scopedPerUser()
        .build(),
    ];
