...
export function useCreateCommunity() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      name: string;
      description: string;
      coverImageHash: ExternalBlob | null;
    }) => {
      if (!actor) throw new Error("Actor not ready");
      return actor.createCommunity(name, description, coverImageHash);
    },
    onSuccess: () => {
...
export function useListCommunities() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) throw new Error("Actor not ready");
      return actor.listCommunities(null, DEFAULT_PAGE_SIZE);
    },
    enabled: !!actor && !isFetching,
  });
}
...
export function useSearchCommunities(query: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) throw new Error("Actor not ready");
      return actor.searchCommunities(query, 20n);
    },
    enabled: !!actor && !isFetching && query.trim().length > 0,
  });
}
...
export function useGetCommunity(id: CommunityId | null) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const result = await actor.getCommunity(id);
      return result ?? null;
    },
...
export function useCreateGroup() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not ready");
      return actor.createGroup(communityId, name, description, coverImageHash);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["groupsByCommunity", variables.communityId.toString()],
      });
...
export function useListGroupsByCommunity(communityId: CommunityId | null) {
  const { actor, isFetching } = useActor(createActor);

  return useInfiniteQuery({
    queryKey: ["groupsByCommunity", communityId?.toString()],
    queryFn: async ({ pageParam }) => {
      if (!actor || communityId === null) throw new Error("Actor not ready");
      return actor.listGroupsByCommunity(
        communityId,
        pageParam,
        DEFAULT_PAGE_SIZE,
      );
...
export function useSearchGroups(query: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) throw new Error("Actor not ready");
      return actor.searchGroups(query, 20n);
    },
    enabled: !!actor && !isFetching && query.trim().length > 0,
  });
}

export function useGetGroup(id: GroupId | null) {
  const { actor, isFetching } = useActor(createActor);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor || id === null) throw new Error("Actor not ready");
      const result = await actor.getGroup(id);
      return result ?? null;
    },
    enabled: !!actor && !isFetching && id !== null,
  });
...
export function useGetGroupMembers(groupId: GroupId | null) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return actor.getGroupMembers(groupId, 0n, 100n);
    },
    enabled: !!actor && !isFetching && groupId !== null,
  });
}

export function useIsGroupMember(
  ...
  return useQuery({
    queryKey: ["isGroupMember", groupId?.toString(), member?.toString()],
    queryFn: async () => {
      if (!actor || groupId === null || !member)
        throw new Error("Actor not ready");
      return actor.isGroupMember(groupId, member);
    },
    enabled: !!actor && !isFetching && groupId !== null && !!member,
  });
}
...
export function useCreateGroupPost() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not ready");
      return actor.createGroupPost(groupId, text, mediaHash, mediaType);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["groupFeed", variables.groupId.toString()],
      });
    },
  });
}

export function useGetGroupFeed(groupId: GroupId | null) {
  const { actor, isFetching } = useActor(createActor);

  return useInfiniteQuery({
    queryKey: ["groupFeed", groupId?.toString()],
    queryFn: async ({ pageParam }) => {
      if (!actor || groupId === null) throw new Error("Actor not ready");
      return actor.getGroupFeed(groupId, pageParam, DEFAULT_PAGE_SIZE);
    },
    initialPageParam: null as bigint | null,
    getNextPageParam: (lastPage) => lastPage.nextCursor ?? undefined,
    enabled: !!actor && !isFetching && groupId !== null,
...
export function useGetGroupPost(postId: bigint | null) {
  const { actor, isFetching } = useActor(createActor);

  return useQuery({
    queryKey: ["groupPost", postId?.toString()],
    queryFn: async () => {
      if (!actor || postId === null) throw new Error("Actor not ready");
      const result = await actor.getGroupPost(postId);
      return result ?? null;
    },
    enabled: !!actor && !isFetching && postId !== null,
  });
